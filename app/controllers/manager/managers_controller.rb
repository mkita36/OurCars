class Manager::ManagersController < ApplicationController
  ...
  def create
    ...
    if @manager.save
      redirect_to company_manager_managers_path(@company), notice: '新規登録完了'
    else
      render :new
    end
  end
  ...
  def update
    if @manager.update(manager_params)
      redirect_to company_manager_managers_path, notice: '更新完了'
    else
      render :edit
    end
  end
  ...
  def destroy
    if @company.managers.count == 1
      redirect_to company_manager_managers_path, notice: '管理ユーザーを0人にすることはできません。'
    else
      @manager.destroy
      redirect_to company_manager_managers_path, notice: '削除完了'
    end
  end
